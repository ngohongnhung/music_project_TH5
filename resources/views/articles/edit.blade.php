...
<div class="container">
    ...
    <form action="{{ route('articles.update', $article->ma_bviet) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="ten_bhat" name="ten_bhat" value="{{ $article->ten_bhat }}" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="ma_tgia" name="ma_tgia" required>
                ...
                @foreach ($authors as $author)
                    <option value="{{ $author->id }}" {{ $author->ma_tgia == $article->ma_tgia ? 'selected' : '' }}>{{ $author->ten_tgia }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="ma_tloai" name="ma_tloai" required>
                ...
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}" {{ $category->ma_tloai== $article->ma_tloai ? 'selected' : '' }}>{{ $category->ten_tloai }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="tomtat" name="tomtat" rows="3" required>{{ $article->tomtat }}</textarea>
        </div>
        ...
        <button type="submit" class="btn btn-primary"><i class="bi bi-pencil-square"></i> Update Article</button>
